    ...
    for index, row in gdf.iterrows():
        ...
        if not overlapping_rows.empty:
            ...
            if pol_area > max(overlapping_rows_area):
                ...
                result_gdf.drop(index=overlapping_rows.index, inplace=True)
                
        # Get pin information from the child polygons
        pin_list = overlapping_rows[pin].tolist()
...
def merge_by_filled_polygon(gdf):
    """
...
def flatten_cadastre(cad_gdf, 
    ...
    if merge_by_filled_polygon:
        gdf = merge_by_filled_polygon(gdf)
